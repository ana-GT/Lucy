...
const string HopeTab::leftArm_LastLink_name_ =  "LJ6";
const string HopeTab::robot_name_ = "Robina";
...
BEGIN_EVENT_TABLE( HopeTab, wxPanel )
EVT_COMMAND ( wxID_ANY, wxEVT_RST_SLIDER_CHANGE, HopeTab::OnSlider )
EVT_COMMAND ( wxID_ANY, wxEVT_COMMAND_BUTTON_CLICKED, HopeTab::OnButton )
END_EVENT_TABLE()

// Class constructor for the tab: Each tab will be a subclass of RSTTab
IMPLEMENT_DYNAMIC_CLASS( HopeTab, RSTTab )

/**
 * @function HopeTab
 * @brief Constructor
 */
HopeTab::HopeTab( wxWindow *parent, const wxWindowID id,
	...
	wxStaticBox* configBox = new wxStaticBox( this, -1, wxT("Enter info") );
	...
	col1Sizer->Add(new wxButton(this, button_showStart, wxT("Show S&tart")),
			...
			wxALL, // make border all around (implicit top alignment)
			1); // set border width to 1, so start buttons are close together
	col1Sizer->Add(new wxButton(this, button_free, wxT("Free button")),
			0, // make horizontally unstretchable
			wxALL, // make border all around (implicit top alignment)
			1); // set border width to 1, so start buttons are close together
	...
	col3Sizer->Add(new wxButton(this, button_showGoal, wxT("Show G&oal")),
			...
			wxALL, // make border all around (implicit top alignment)
			1); // set border width to 1, so start buttons are close together
	col3Sizer->Add(new wxButton(this, button_test, wxT("Test -- Free")),
			0, // make horizontally unstretchable
			wxALL, // make border all around (implicit top alignment)
			1); // set border width to 1, so start buttons are close together
 ...
 */
void HopeTab::initTrans()
{
   Eigen::Transform<double, 3, Eigen::Affine> basePose = world->robots[robot_ID_]->baseLink->absPose;
   Eigen::VectorXd baseTrans = basePose.translation();


   TWBase_.setIdentity();
   TWBase_.rotate( AngleAxis<double>( 0*PI, Vector3d(0.0, 0.0, 1.0) ) );
   TWBase_.translation()<< baseTrans(0),baseTrans(1) , baseTrans(2);

   Tee_.setIdentity(); 
   Tee_.translation()<< -0.070, 0.106, -0.111;
   Tee_.rotate( AngleAxis<double>( PI/2.0, Vector3d(0.0, 1.0, 0.0) ) );
 ...
 */
int HopeTab::getRobotID( string robotName )
{
 ...
 */
void HopeTab::getLinks()
{
  links_ID_.clear();

  int lastLink_ID = world->robots[robot_ID_]->findLink( leftArm_LastLink_name_ );

  links_ID_.resize( leftArm_numLinks_ );
  links_ID_[ leftArm_numLinks_  - 1 ] = lastLink_ID;
  for(int i = leftArm_numLinks_ - 2; i >= 0; i--) 
    { links_ID_[i] = world->robots[robot_ID_]->links[links_ID_[i+1]]->parent->index; }

  num_links_ = links_ID_.size();

  EE_ID_ = links_ID_[links_ID_.size() - 1];
}
 ...
 */
Eigen::VectorXd HopeTab::getCurrentConfig()
{
  Eigen::VectorXd current_config;
  current_config.resize( num_links_ );

  for( int i = 0; i < num_links_; i++ )
     { current_config[i] = world->robots[robot_ID_]->links[links_ID_[i]]->jVal; }
 ...
 * @author Achq 
 */
void HopeTab::OnButton(wxCommandEvent &evt) {

        //-- ALWAYS initialize
        robot_ID_ = getRobotID( robot_name_ );
        getLinks();
        initTrans();
	...
	case button_SetStart:
                if( world != NULL )
                  { if( world->robots.size() < 1 )
                    ...
                    start_config_.resize( num_links_ );
                    for( int i = 0; i < num_links_; i++ )
                       { start_config_[i] = world->robots[robot_ID_]->links[ links_ID_[i] ]->jVal;
                         cout<<start_config_[i]<<" ";
                       }
                    cout<<endl;
                  } 
                else
	...
	case button_SetGoal:
		if( world != NULL )
                  { if( robot_ID_ < 0 )
                    ...
                    if( selectedObject != NULL )
                      { 
                        goal_ID_ = world->findObject( selectedObject->name );
			goal_pose_.setIdentity();
                        goal_pose_ = selectedObject->absPose; 
                        goal_position_.resize(3);
                        goal_position_ = goal_pose_.translation();
                        goal_name_ = selectedObject->name;

                        cout<<"** Goal is: "<<goal_name_<<" with position: "<<endl;
                        cout<<goal_position_<<endl;
                      }
                    else
        ...
        case button_ShowListMovObj:
                ...
                cout<<"** Number of Movable Objects registered:" << movableObjects_.size()<<endl;

		for( int i = 0; i < movableObjects_.size(); i++ )
                   {
                     cout<<"["<<i<<"] "<<world->objects[ movableObjects_[i] ]->name << endl;
                   } 
		break;        
        ...
        case button_CheckCollisions:
	       ...
	       world->updateAllCollisionModels();
               if( world->checkCollisions() == false )
                 { cout << "--(v) No Collisions detected " << endl; }
               else
                 { cout << "--(x) Collisions detected" << endl; }
              } 
               break; 
	...
	case button_showGoal:
                ...
                if( goal_position_.size() < 1 )
                  { cout<<"--(!) You have not set a Goal position yet"<<endl; break; }

                cout<<" ** The goal object is: "<<goal_name_<<" and its location is:"<<endl<<goal_position_ << endl;                
	...
	case button_free:
               { vector<Eigen::VectorXd> path;
                 ...
                 /* 
                 goWSOrient a;  
                 a.initialize( world, robot_ID_, links_ID_, TWBase_, Tee_ );
                 result = a.balancePath( start_config_, goal_pose_, path );
                 
                 goWSOrient a;  
                 a.initialize( world, robot_ID, links_ID, TWBase, Tee );
                 result = a.straightPath( startConfig, goal_pose, path );
                 */
                 if( result == true )
                   { SetTimeline( robot_ID_, links_ID_, path ); }                 
               } 
		break;
	...
	case button_test:
               {
                 ...
                 robinaLeftArm_fk( getCurrentConfig(), TWBase_, Tee_, T );
	...
	case button_Plan:
               {   
                ...
                HopePlanner hope;
                hope.initialize( *world, robot_ID_, links_ID_, TWBase_, Tee_ );  
                result = hope.findPath( start_config_, goal_pose_, path );                
 ...
 */
void HopeTab::SetTimeline(int robot, vector<int> links, vector<Eigen::VectorXd> path){
  ...
  for( int i = 0; i <path.size(); i++ ) 
     { 
       world->robots[robot]->setConf( links, path[i] ); 
...
/**
 */
void HopeTab::SetTimeline(int robot, vector<int> links, list<Eigen::VectorXd> path){
		if(world == NULL ||  path.size() == 0){
			cout << "Must create a valid plan before updating its duration." << endl;
			return;
		}
 ...
 */
void HopeTab::OnSlider(wxCommandEvent &evt) {
 ...
 */
void HopeTab::reset_all()
{
  movableObjects_.clear();
}
