 ...
 */
void CheckObject::getModelData( Object* object )
{
  getModelTriangles( object );
  getModelCenter();
  getModelRadius();
  getModelBoxVertices();
}
 ...
 */
void CheckObject::getModelTriangles( Object* object )
{
 ...
 */
void CheckObject::updateObjectData( Object* object, BinaryVoxel* voxel )
{
  reset_minmax();

  //-- Get the current orientation of the objects
  Eigen::Vector3d trans = object->absPose.translation();
  Eigen::Matrix3d rot = object->absPose.rotation().matrix(); 
  ...
  {  Eigen::Vector3d new_ver = rot * Eigen::Vector3d( this->boxVer[i][0], this->boxVer[i][1], this->boxVer[i][2] ) + trans ;
     ...
     //-- Get the vertices in voxels
     voxel->world2VoxelCell( this->current_boxVer[i][0], this->current_boxVer[i][1], this->current_boxVer[i][2], 
			     this->current_boxVerVoxels[i][0], this->current_boxVerVoxels[i][1], this->current_boxVerVoxels[i][2] ); 
  }
