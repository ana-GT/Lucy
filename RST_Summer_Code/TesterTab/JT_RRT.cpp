 ...
 */
void JT_RRT::initialize( World* _world, 
		    	const int &_robot_ID, 
 ...
 * @brief
 */
bool JT_RRT::plan( const Eigen::VectorXd &_startConfig, 
                   ...
                   std::vector<Eigen::VectorXd> &path )
{
  ...
  if( goalDistVector[activeNode] < distanceThresh ) 
    { tracePath( activeNode, path );
      cout<<"Got a path! - nodes: "<<path.size()<<" tree size: "<<configVector.size()<<endl; 
      return true; }
  else
    { cout<<"No successful path found! "<<endl;
      return false; }
}
 ...
 * @function wsDistance
 */
double JT_RRT::wsDistance( Eigen::VectorXd q )
{
 ...
 * @function wsDiff
 */
Eigen::VectorXd JT_RRT::wsDiff( Eigen::VectorXd q )
{
  Eigen::Transform<double, 3, Eigen::Affine> T;
  robinaLeftArm_fk( q, TWBase, Tee, T );
  Eigen::VectorXd ws_diff = ( goalPosition - T.translation() );
 ...
 * @brief 
 */
bool JT_RRT::connect( const Eigen::VectorXd &target )
{
 ...
 * @function tryStep
 */
JT_RRT::StepResult JT_RRT::tryStep()
{
  Eigen::VectorXd qtry = getRandomConfig();
  return tryStep(qtry);
}
 ...
 * @function tryStep
 */
JT_RRT::StepResult JT_RRT::tryStep( const Eigen::VectorXd &qtry )
{
 ...
 * @function tryStep
 */
JT_RRT::StepResult JT_RRT::tryStep( const Eigen::VectorXd &qtry, int NNidx )
{
 ...
 * @function addNode
 */
int JT_RRT::addNode( const Eigen::VectorXd &qnew, int parent_ID)
{
 ...
 */
int JT_RRT::getNearestNeighbor( const Eigen::VectorXd &qsample )
{
 ...
 */ 
Eigen::VectorXd JT_RRT::getRandomConfig()
{
 ...
 * @function tracePath
 */   
void JT_RRT::tracePath( int node_ID, std::vector<Eigen::VectorXd> &path, bool reverse )
{
  int x = node_ID;
 ...
 */
bool JT_RRT::checkCollisions( const Eigen::VectorXd &config )
{
 ...
 */
void JT_RRT::add_Ranking( int node_ID )
{
  int n; 
  int node; int parent;
  int temp;
